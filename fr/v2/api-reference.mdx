...
<Note>
  **Clé API** : Obtenez votre clé API depuis les paramètres développeur dans votre tableau de bord.
</Note>
...
<CodeGroup>
```bash cURL
curl -H "Authorization: Bearer $SALSA_API_KEY" \
  https://api.salsaasaservice.com/v2/ingredients
```

```javascript JavaScript
const headers = {
  Authorization: `Bearer ${process.env.SALSA_API_KEY}`
};
```
...
headers = {"Authorization": f"Bearer {os.environ['SALSA_API_KEY']}"}
```
</CodeGroup>
